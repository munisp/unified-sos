...
| KYC-KYB | KYC/KYB / document AI / liveness (SPEC-KYC-KYB) | KYC & KYB case management, document artifacts + extraction (PaddleOCR/Docling/VLM adjudication), liveness challenges/anti-spoof, CAC/NIMC/tax/sanctions registry verification, beneficial ownership, risk scoring, review queues, hash-chained audit | `services/mod-kyc-kyb/`, `db/migrations/0005_kyc_kyb.sql`, `config/states/*/modules.yaml` | `services/mod-kyc-kyb/tests/`; `contracts/openapi/mod-kyc-kyb.yaml`; channels `ng.sos.kyc.case_decision_made`, `ng.sos.kyb.case_decision_made`, `ng.sos.kyc.liveness_failed`, `ng.sos.kyb.registry_verification_completed` | IMPLEMENTED (reference impl.; PaddleOCR/Docling/VLM + live registry bindings are ADAPTER-SEAM, fail-closed) |
...
| GEO-06 | Stage 5 geospatial orchestration: dataset registry, H3 indexing, processing jobs, lakehouse publication; PostGIS `geospatial` schema as system of record | `services/mod-geospatial/`, `db/migrations/0006_geospatial.sql`, `contracts/openapi/mod-geospatial.yaml`, `config/states/*/modules.yaml` | `services/mod-geospatial/tests/`; channels `ng.sos.geospatial.*`; RLS policies | IMPLEMENTED (reference impl.; live Sedona/PostGIS/Delta runtimes are ADAPTER-SEAM) |
| GEO-07 | Stage 5 low-latency geospatial validation & job command gateway (Go) | `services/mod-geospatial-gateway/` | Go tests (`go test ./...`) | IMPLEMENTED (local reference; Temporal job runtime binding pending) |
| GEO-08 | Stage 5 deterministic geometry validation primitives (Rust `geometry-rs`) | `geospatial/geometry-rs/` | 31 Rust tests (merged; CI `rust` job — cargo execution unavailable in authoring sandbox, static-reviewed) | IMPLEMENTED |
| GEO-09 | GeoLibre sovereign self-hosted GIS workbench (never system of record; no hosted share/collab) | `deploy/docker-compose.yml` (`geolibre`, `ghcr.io/opengeos/geolibre:2.0`, port 8085), `services/mod-geospatial/` project builder, `geospatial.geolibre_projects` table | channel `ng.sos.geospatial.geolibre_project_built` | IMPLEMENTED (self-hosted; `geolibre_wasm` processing adapter is ADAPTER-SEAM, fail-closed) |
...
| SEC-01 | Zero cross-tenant leakage: `tenant_state_id` scoping + Postgres RLS on all tenant tables | `db/migrations/0001–0005`, service repositories | RLS policies; pen-test gate (REQ-SEC-01) | IMPLEMENTED |
