...
## 16. Stage 13 — Land Management best-of-both (munisp/landmanagement gap closure)
- Gap analysis: docs/delivery/landmanagement-gap-analysis.md (IDL­R-PTS 105-feature TS monorepo reviewed; platform already ahead on titling workflow, signing, LUC, AVM, geospatial backbone).
- NEW services/mod-land-docs: document lifecycle + OCR seam (PaddleOCR/Docling fail-closed), classification, verification, versioning/supersede, duplicate detection, hash-chained audit — 39 tests.
- NEW services/mod-mortgage: application → credit review (credit_mlp seam) → lien (priority, second charge) → two-phase disbursement (idempotent, integer kobo, conservation) → annuity schedule (exact kobo, remainder last) → payments oldest-first → atomic discharge / default / foreclosure — 58 tests.
- mod-gis-lands extensions: area-conserving subdivision/merger with lineage, dispute lifecycle blocking titling/subdivision (409), chain-of-title history, title-risk seam (fraud_gnn), merkle anchor/verify seam — +49 tests (86 total).
- Wiring: OpenAPI specs (mod-land-docs 10 ops, mod-mortgage 12 ops), AsyncAPI +13 channels (registry 49 artifacts OK), validate_packs 37 PASSED, compose 8022/8023, Helm modLandDocs/modMortgage, Prometheus jobs.
